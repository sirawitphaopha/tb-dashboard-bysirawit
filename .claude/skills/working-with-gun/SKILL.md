...
- 🚨🆕 **เช็ค Chrome จริง — แต่ไม่ต้องเทสทุกครั้ง (พี่กันสั่ง 5 ก.ค. 69)** — พี่กันชมว่าเทสละเอียดดี แต่**มันช้า** (บางทีพี่กันไปกินข้าว) · **เทสเองเฉพาะ: (1) พี่กันสั่งให้เทส (2) จุดที่เป็นระบบ/ฟีเจอร์ใหม่ (เทสเองได้) — นอกนั้นถามพี่กันก่อน หรือให้พี่กันเทสเอง** · อย่าเทส Chrome จบทุกการแก้เล็กๆ · เมื่อจะเทส = อย่าเชื่อการคิดในหัว ทำตามนี้:
  - 🔴🔴 **หน้าที่เป็นส่วนหนึ่งของแอปจริง (มี auth/ข้อมูลจริง) = ต้องเทส "หน้าจริง" ในเบราว์เซอร์จริง ห้ามสร้าง mock HTML มาเทสแทนเด็ดขาด** (พลาดหนัก 5 ก.ค. 69 · พี่กันโมโหมาก "มาสร้างตัวเทสทำไม เปิดโลคอลจริงๆ" · mock ผ่านหมดแต่เว็บจริงมีบั๊กตัวนับ) → วิธี: `curl localhost:3000` เช็ค dev รันอยู่ → **claude-in-chrome MCP** (`switch_browser`/`select_browser` เชื่อม Chrome ที่พี่กันล็อกอินไว้ · ต้องถาม AskUserQuestion เลือก browser ก่อน) → `navigate` + คลิกไปหน้าจริง → `computer screenshot`/`zoom` + `read_page` ดูค่าเรนเดอร์จริง + กดปุ่ม/ทอกเกิลเทส interaction (ตัวนับ/กรอง/ยุบ) → เจอบั๊กแก้ในโค้ด → dev HMR โหลดเอง → รีเฟรชหน้าจริงยืนยันซ้ำ
  - 🔴 **เทสที่สัดส่วนจริง = เดสก์ท็อป landscape มี sidebar (กว้าง ~1360px)** ไม่ใช่ portrait/จอแคบ (เว็บ TB แดช = เดสก์ท็อป · Daily Quest เท่านั้นที่ mobile portrait)
  - mock `public/xxx.html` + static server = ใช้ได้ **เฉพาะ** ทดสอบ layout primitive แยกเดี่ยว ที่เข้าหน้าจริงไม่ได้ (เช่น สูตร sticky) · และต้องเทสที่สัดส่วนจริงด้วย · **ห้ามเอา mock มาสรุปแทนหน้าจริง**
  - flow เดิม (ถ้าต้องใช้ mock): `preview_start` static (python http.server ชี้ `public/`) → `preview_eval` วัด `getBoundingClientRect` + `preview_screenshot` → ลบไฟล์เทส
...
- ❌🆕 **เคลม "บั๊กหาย" จากค่าภายใน (transform tx/ตัวเลข JS) ทั้งที่ลากจริงยังไม่ขยับให้เห็น** (พลาดหนัก 6 ก.ค. 69 · พี่กันจับได้ "มาบอกว่าหายได้ไงวะ") → **ต้องพิสูจน์ด้วยพฤติกรรม/ภาพจริง** ก่อนบอกว่าหาย · **user บอกเงื่อนไขเทส (เช่น "ซูม 1000%") = เทสตามนั้น ห้ามเปลี่ยนเป็น 200% เอง** · แก้เสร็จ HMR อาจยังไม่เข้า tab ของ user → บอก **Ctrl+Shift+R (hard refresh)** ก่อนสรุปว่าแก้ไม่ได้ผล
- 🆕 **เทคนิค AvatarLightbox + SVG:** SVG ไม่มี width/height (`naturalWidth` = 200/0 ไม่ตรงขนาดจริง) → ขอบเขตลาก (baseSize) ต้องใช้ **`imgRef.current.offsetWidth/offsetHeight` (ขนาดเรนเดอร์จริง)** ไม่ใช่ naturalWidth/dim · **เทส pan อัตโนมัติ: CDP left_click_drag แพนรูปในตัวดูรูปไม่ได้ (ทั้งรูปถ่าย/SVG)** → ต้อง `dispatchEvent` mousemove บน div backdrop (zIndex 10001) แล้ววัด `getBoundingClientRect().left` ก่อน/หลัง
