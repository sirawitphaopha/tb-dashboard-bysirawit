...
- **ประวัติรูปภาพ (audit log) + ตรวจรูปซ้ำ** — บันทึก **ทุก event ของรูป** (อัปโหลด/แก้/ขอลบ/ยกเลิก/แอดมินลบตรง/อนุมัติ/ปฏิเสธ/กู้คืน/ลบถาวร/auto-purge) ลงตาราง `tb_image_event_log` (jsonb snapshot · **เก็บถาวรแม้รูปลบถาวร**) · ดูที่หน้าคลังรูปภาพ กด toggle "ประวัติ" (แอดมิน) มีตัวกรอง event/วันที่ + ไทม์ไลน์ + ดู snapshot · ตอนอัปคำนวณ **SHA-256 / MD5 / CRC32 (ต้นฉบับ) + pHash (dHash · จับภาพเดียวกันแม้คนละไฟล์)** เก็บไว้ตรวจรูปซ้ำ · **v0.7.22 ยกเครื่องหน้าประวัติ:** โชว์**รูปย่อจริง**ทั้งมุมตามรูป/ตามเวลา (รูปลบถาวร = ไอคอนไฟ) · **กดรูปย่อดูรูปใหญ่ได้แบบเดียวกับคลัง** (ซูม/หมุน · กันคลิกขวา/บันทึกภาพ) · SHA-256 ค่าเต็ม + ไฮไลต์เลขวิเศษ · ป้ายสีแยก**แอดมิน/ผู้ใช้** + คำว่า "หมายเหตุ" · **กดยุบกิจกรรม/ยุบทั้งหมด** · **แถบสรุปยอด**กดกรองได้ · กดป้าย "ภาพคล้าย" กรองย่อยเฉพาะกลุ่ม · ปุ่มกระโดดไป **"คลัง"** และ **"เวชระเบียนคนไข้"** · แก้บั๊กลากรูป SVG ตอนซูมสูง
...
- **คลังความรู้วัณโรค (ระบบ PDF) — v0.8** — เก็บไฟล์ PDF (ไกด์ไลน์/Trial/อื่นๆ) บน R2 อ่านในเว็บได้เลยด้วยตัวอ่าน PDF เต็มรูปแบบ (toolbar เทลแบบ Chrome · เลื่อนหน้า/ซูม/หมุน/พอดีหน้า-กว้าง/2หน้า/หน้าย่อลากปรับได้/ค้นหา+ไฮไลต์ไทย/เลือก-คัดลอก/ดาวน์โหลด/พิมพ์/นำเสนอ/เปิดแท็บใหม่/คุณสมบัติเอกสาร+แฮช+อัปจากไหน) · ทุกคนอ่านได้ · แอดมินอัป/ลบ (optimistic 2 ขั้น) · ทัมเนลหน้าแรกอัตโนมัติ (2K) + ปรับขนาด 3 ระดับ + realtime ข้ามเครื่อง · เข้าจากหน้าคลังความรู้เดิม
...
R2_BUCKET_LIBRARY                  # v0.8 คลังความรู้ PDF (เช่น tb-knowledge · ถังแยก · ต้องตั้ง CORS เอง)
...
**รูปแบบเวอร์ชัน = 4 ตำแหน่งเสมอ `X.Y.Z.W`** (ตั้งกฎ 5 ก.ค. 2569) — เติม `.0` ถ้าขาด (`0.7.21` → เขียน `0.7.21.0`) · ไม่เกิน 4 ตำแหน่ง (`0.7.21.2.5` → `0.7.21.2`) · เวอร์ชันที่ push ไปแล้วไม่แก้ย้อนหลัง
