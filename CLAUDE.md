...
R2_BUCKET_LIBRARY                 # v0.8 — knowledge PDF library (separate bucket, e.g. tb-knowledge; set CORS on it too)
...
### คลังความรู้วัณโรค (ระบบ PDF) — v0.8 (v0.8.0.1)
- **เป้าหมาย:** เก็บไฟล์ PDF (ไกด์ไลน์/Trial/อื่นๆ) บน R2 ของเว็บเอง แล้ว **อ่านในเว็บได้เลย** · ทุกคนที่อนุมัติ **อ่าน** ได้ · แอดมินเท่านั้น **อัป/ลบ** · เข้าจากหน้า "คลังความรู้วัณโรค" เดิม (ปุ่ม "เปิดคลัง PDF" ใน `parts/misc.jsx` `KnowledgeBase` → state `knowledgeView` ใน `tb-monolith.jsx` สลับ home↔pdf) — **ปล่อย `KnowledgeBase` placeholder เดิมไว้** (ไว้ทำระบบเขียนบทความในอนาคต · idea 5C)
- **DB:** ตาราง `tb_knowledge_docs` (`scripts/add-knowledge-library.sql`) — `category` (guideline/trial/other), title, file_name, source_url, storage_key, thumb_key, mime, size_bytes, page_count, device (v0.8.0.2 · `scripts/add-knowledge-device.sql`), uploaded_by, uploaded_at · **ไม่มี soft-delete (ลบจริง · hard delete)** เพราะไม่ใช่ข้อมูลคนไข้ · RLS: `is_approved()` อ่าน / `is_admin()` เขียน-ลบ · **เปิด realtime** (`scripts/add-knowledge-realtime.sql` — เพิ่มตารางเข้า publication `supabase_realtime`)
- **R2:** ถังแยกใหม่ `tb-knowledge` (env `R2_BUCKET_LIBRARY` · default ในโค้ด = `tb-knowledge` ตรงชื่อถังจริง) · key `library/<uuid>.pdf` + `library/<uuid>_thumb.webp` (ปกหน้าแรก) · `lib/knowledge-helpers.ts` (`LIBRARY_BUCKET` + re-export `getRequester`) · 🚨 **ถังใหม่ต้องตั้ง CORS เอง** (allow localhost:3000/3001 + LAN 192.x + tbjourney.care · GET/PUT/HEAD) ไม่งั้นเบราว์เซอร์อัปไม่ได้ = "เครือข่ายมีปัญหา" (`xhr.onerror`)
- **API `app/api/knowledge/`:** `presign` (POST admin → PUT url PDF + thumb · 900s), `confirm` (POST admin → insert row), `route.ts` (GET approved `?category=&q=` → docs + `url`/`thumbUrl` signed + ชื่อคนอัป join profiles · q sanitize กัน `.or()` พัง), `[id]/url` (GET approved → signed url สด 3600s + fileName), `[id]` (DELETE admin → `r2Delete` PDF+thumb → ลบ row) · ทุก route guard 401 `!user` / 403 `!isApproved` / (403 `!isAdmin` เฉพาะ write) + เขียนผ่าน `createAdminClient()`
- **ตัวอ่าน PDF (pdf.js `pdfjs-dist` 6.1.200):** ประกอบ viewer เอง (`pdfjs-dist/web/pdf_viewer.mjs`: `EventBus`/`PDFViewer`/`PDFLinkService`/`PDFFindController`) — ⚠️ **บิลด์นี้ไม่มี `PDFThumbnailViewer` → หน้าย่อทำเอง** (render หน้าเล็ก lazy ด้วย IntersectionObserver) · worker (`pdf.worker.min.mjs`) + `pdf_viewer.css` คัดลอกเข้า `public/` โดย `scripts/copy-pdf-worker.mjs` (postinstall/prebuild · **gitignored**) · โหลด css ผ่าน `<link href="/pdf_viewer.css">` runtime (เลี่ยงข้อห้าม Next.js import css จาก node_modules) · `GlobalWorkerOptions.workerSrc='/pdf.worker.min.mjs'` (same-origin) · `getDocument({isEvalSupported:false})` · scroll-lock body + StrictMode-safe (cancelled flag + task/pdf/viewer destroy ตอน cleanup) · l10n ปล่อย default (PDFViewer สร้าง `GenericL10n` เอง) · CSP ไม่ต้องแก้ (worker-src/connect-src/wasm-unsafe-eval ครบ · `<iframe>/<embed>` ใช้ไม่ได้ → canvas เท่านั้น)
- **Frontend `parts/knowledge/`:** `helpers.jsx` (`KNOWLEDGE_CATEGORIES` + `getPdfjs` lazy + `renderPdfCover` render หน้าแรก WebP **2560px/คุณภาพ 0.8** + นับหน้า · best-effort ล้มก็อัปได้ · re-export `putWithProgress`/`fmtFileSize`/`loadCache`/`saveCache`), `upload-modal.jsx` (`KnowledgeUploadModal` admin · presign→`putWithProgress`(%จริง)→PUT ปก→confirm), `library-page.jsx` (`KnowledgeLibraryPage`), `pdf-viewer.jsx` (`PdfViewer`), barrel `knowledge.jsx`
- **`library-page.jsx` จุดสำคัญ:** โหลดแบบ **seed-cache + revalidate เงียบเสมอ** (`loadCache` โชว์ทันที ไม่ skeleton → fetch ใหม่ทุกครั้งเงียบ ๆ · **กันค้างของเก่าข้ามเครื่อง** · บทเรียน 192 ไม่เห็น PDF) · **realtime** subscribe `tb_knowledge_docs` (debounce 500ms → `load()`) · **ลบ optimistic** (ป๊อป 2 ขั้น SWAPPED · กดยืนยัน = ปิดป๊อป+เอาการ์ดออกทันที → DELETE เบื้องหลัง · ล้มเหลว = คืนการ์ด + แถบ flash) · **ปรับขนาดทัมเนล 3 ระดับ** (`CARD_SIZES` min 150/205/280 · grid `auto-fill minmax` · จำใน `localStorage` `tb_libknow_size`) · ทัมเนลการ์ด **แนวตั้ง A4** (`aspectRatio 210/297` + `objectFit contain`) · sticky header สูตรบ้านนี้ (`top:-24px`)
- **`pdf-viewer.jsx` (`PdfViewer`) — โฉม Chrome (v0.8.0.2):** toolbar **เทล** ปุ่ม**อยู่กลาง** · เปิดมา **100%** (`pagesinit` → `currentScale=1`) · ซูม dropdown presets (พอดีกว้าง/พอดีหน้า/50–500%) + `+/−` ทีละขั้น (×1.2 · เพดาน pdf.js `MAX_SCALE=25` = 2500% · 4000% ต้องซ้อน CSS overzoom ยังไม่ทำ) · **ปุ่ม fit toggle เดียว** สลับพอดีกว้าง(↔)↔พอดีหน้า(↕) (`fitNext`) · **2 หน้า** (`spreadMode`) · **นำเสนอ** (`requestFullscreen`) · **เปิดแท็บใหม่** (native · ขอ url สด) · **คุณสมบัติเอกสาร** (`DocProps` **พื้นขาว** · `pdf.getMetadata` Producer/PDFFormatVersion/วันที่ + numPages + ขนาดหน้า(มม.) + คนอัป/วันที่/**device** + **แฮชสด** SHA-256/MD5/CRC32 จาก `pdf.getData()`→`computeByteHashes`) · ปุ่มคุณสมบัติมีบน toolbar ตรงๆ + เมนู ⋮ · **แถบหน้าย่อลากปรับกว้างอิสระ** (`thumbW` + drag handle) · find bar **เทลโปร่งใส+เบลอ** · **บาร์ % โหลด** (`loadingTask.onProgress`) · **เปิดเร็ว** (ใช้ `doc.url` ที่มี + retry ขอสดถ้าหมดอายุ · preload `getPdfjs()` ตอน mount) · animation เด้งเข้า → **เอา transform ออกหลังจบ (แก้ตัวอักษรเบลอ)** · **device** เก็บตอนอัป (`detectDevice()` → confirm → คอลัมน์ `device`)
