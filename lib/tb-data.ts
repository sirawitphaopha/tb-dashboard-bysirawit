...
export const DRUG_RANGES: Record<string, { name: string; strength: number; min: number; max: number; absMax: number }> = {
...
export type LabField = { key: string; label: string; unit: string; lo: number; hi: number; critical: number | null; lowBad?: boolean };
export type LabGroup = { id: string; label: string; color: string; fields: LabField[] };

export function getLabStatus(val: unknown, field: LabField): string {
...
export function calcDoses(weight: number | string, regimen: string, manualTabs: Record<string, number> | null): DoseResult[] {
  ...
  return keys.map(key => {
    ...
    const st: 'ok'|'low'|'high' = (mgkg > d.max || dose > d.absMax) ? 'high' : (mgkg < d.min) ? 'low' : 'ok';
    return { key, name:d.name, strength:d.strength, tabs:t, dose, mgkg, min:d.min, max:d.max, absMax:d.absMax, status:st };
  });
}

export function calcCrCl(age: number, weight: number, scr: number, gender: string): number | null {
...
export function crClStage(crcl: number | null): { label: string; color: string } {
...
export function migrateAdr(adr: Record<string, unknown>): Record<string, {checked: boolean; note: string}> {
  if (!adr) return {};
  ...
  ADR_LIST.forEach(a => {
    ...
    else                               { out[a.key] = v as {checked: boolean; note: string}; }
  });
  return out;
}
...
export const INITIAL_PATIENTS: Patient[] = [
  ...
  },
  {
    ...
    visits:[
      {id:'v1',date:'2024-01-10',weight:58,type:'visit',vitals:{bp:'128/82',hr:'78',rr:'18',temp:'36.7',o2:'98'},drugDoses:'H300 R450 Z1000 E800',note:'S: ปวดหลังลดลงบ้าง ไอแห้งๆ\nO: BW 58kg, BP 128/82\n>>> CXR: RUL infiltration ลดลงเล็กน้อย\n>>> Sputum AFB 2+ | GeneXpert: MTB Detected, RIF Sensitive\n>>> Baseline: AST 28 ALT 32 ALP 88 UA 7.2\nA: เริ่มยา Relapse case ทนยาได้ดี\nP: นัด 1 เดือน ติดตาม LFT + UA',consult:{type:'ปรับขนาดยา (Dose Adjustment)',note:'H300 R450 Z1000 E800 เหมาะสมสำหรับ BW 58kg'},drp:[],adrNoted:[],labData:{},sputumQuick:{}},
      {id:'v2',date:'2024-03-10',weight:59,type:'visit',vitals:{bp:'130/84',hr:'76',rr:'18',temp:'36.6',o2:'97'},drugDoses:'H300 R450 Z1000 E800',note:'S: ปวดข้อเข่ามากขึ้น บวมเล็กน้อย\nO: UA 11.2 → HOLD PZA\n>>> เปลี่ยนสูตรเป็น 2HRE/7HR\nA: Hyperuricemia จาก PZA ระดับสูง\nP: ตัด PZA, ต่อด้วย HRE, ติดตาม UA เดือนหน้า',consult:{type:'ADR Management',note:'Hyperuricemia UA 11.2 — HOLD PZA เปลี่ยนสูตร HRE'},drp:[{type:'C5',note:'ADR Hyperuricemia จาก PZA — UA 11.2 mg/dL ปวดข้อมาก'}],adrNoted:['arthralgia','nausea'],labData:{},sputumQuick:{tp:'M2',result:'Scanty',scantyCount:'2-5/100F',genexpert:'Not Detected',genexpertRif:''}},
      {id:'v3',date:'2024-05-10',weight:60,type:'visit',vitals:{bp:'124/78',hr:'70',rr:'16',temp:'36.4',o2:'99'},drugDoses:'H300 R450 E800',note:'S: อาการปวดข้อดีขึ้นมาก UA ลดลง\nO: UA 7.8, ALT 38 ปกติ, Sputum M4: Negative\nA: ตอบสนองต่อการรักษาดี Sputum Conversion สำเร็จ\nP: ต่อ HRE ครบกำหนด นัดอีก 1 เดือน',consult:{type:'Medication Counseling',note:'แนะนำความสำคัญของการกินยาต่อเนื่อง และสังเกตอาการตาพร่า (Optic Neuritis จาก EMB)'},drp:[],adrNoted:[],labData:{},sputumQuick:{tp:'M4',result:'Neg',scantyCount:'',genexpert:'',genexpertRif:''}}
    ],
    dot:{
  ...
  },
  {
    ...
    sputum:[
      {tp:'M0',date:'2026-01-15',specimenType:'Sputum',afbSamples:[{result:'3+',scantyCount:''},{result:'2+',scantyCount:''},{result:'3+',scantyCount:''}],result:'3+',scantyCount:'',molecType:'GeneXpert MTB/RIF',genexpert:'Detected',mtbResult:'Detected',rifResult:'RIF resistant',genexpertRif:'Resistant',inhResult:'INH resistant',sldResults:{flqs:'Resistant',agcp:'Susceptible',eto:'Susceptible'},igraResult:'',extraLabs:{}},
      {tp:'M2',date:'2026-03-15',specimenType:'Sputum',afbSamples:[{result:'Scanty',scantyCount:'3'},{result:'Neg',scantyCount:''}],result:'Scanty',scantyCount:'3',molecType:'GeneXpert MTB/RIF',genexpert:'Detected',mtbResult:'Detected',rifResult:'RIF resistant',genexpertRif:'Resistant',inhResult:'',sldResults:{},igraResult:'',extraLabs:{}},
    ],
    adr:{
    ...
    visits:[
      {id:'v1',date:'2026-01-15',weight:63,type:'visit',vitals:{bp:'126/80',hr:'78',rr:'18',temp:'37.0',o2:'98'},drugDoses:'H300 R450 Z1000 E800',note:'S: ไอมีเสมหะ น้ำหนักลด 5 kg ใน 2 เดือน\nO: BW 63kg CXR: RUL cavity + infiltration\n>>> Sputum AFB 3+/2+/3+ | GeneXpert: MTB Detected, RIF Resistant, INH Resistant\n>>> FLQS: Resistant → pre-XDR-TB\nA: MDR-TB + INH resistant + FLQS resistant\nP: ส่งต่อผู้เชี่ยวชาญ MDR-TB clinic ด่วน',consult:{type:'Lab Interpretation',note:'GeneXpert RIF resistant + INH resistant + FLQS resistant → pre-XDR-TB'},drp:[{type:'C2',note:'ต้องการยา SLD regimen — ปรึกษา MDR-TB clinic ด่วน'}],adrNoted:['nausea'],labData:{},sputumQuick:{}}
    ],
    dot:{
