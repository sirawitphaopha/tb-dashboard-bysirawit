...
### 🔭 งานต่อไป (อัปเดต 5 ก.ค. 69 · v0.7.20.3 pushed = feature 6161974 + chore 5efc29f)
...
3. **🟢 #10 ระบบ log รูปภาพ + hash — v0.7.21 (Step 1 เสร็จ · push แล้วเทสหลัง):** ตาราง `tb_image_event_log` (jsonb snapshot ยึด `tb_patients_deleted_log` · เก็บถาวรแม้ hard-delete) + `logImageEvent()` (`lib/image-event-log.ts` fire-and-forget) + **11 จุด route** (confirm/[id]PATCH+DELETE/request-delete POST+DELETE/review-delete/restore/hard/purge/cron-purge-images) · viewer `image-log.jsx` (`ImageLogPage`) **อยู่หน้าคลังรูป** (toggle คลังรูป/ประวัติ · พี่กัน**ย้ายจากถังขยะ**) + ปุ่มลัด "ถังขยะ"→เด้งเปิดแท็บรูป (`trashWantImages` prop · TrashHub useEffect) + API `/api/patient/images/log` (admin service-role) · **hash:** `image-hash.js` (SHA-256 native + MD5/CRC32/dHash **self-contained ผ่าน vector-test**) คำนวณตอนอัป (patient-tab doUpload) → คอลัมน์ `orig_sha256/md5/crc32/phash` (SQL `add-image-hashes.sql`) · snapshot+confirm เก็บ hash ด้วย · ⏳ **เหลือ Step 2-3: view "ตามรูป" (group image_id) + ป้าย/กรอง "รูปซ้ำ" (SHA เป๊ะ + pHash · `phashDistance` Hamming)** · ⚠️ พี่กันยังไม่เทส action
